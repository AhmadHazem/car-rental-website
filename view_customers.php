<?php
  ...
  if(isset($_GET['search']))
  {
    ...
    if ($fname == "" AND $lname =="")
    {
        $myQuery = "SELECT * FROM `user` WHERE isadmin=0;";
        $result = $connect->query($myQuery);
    }
    else
    {
    $myQuery = "SELECT * FROM user WHERE ('$fname' = fname OR '$lname'=lname) AND isadmin=0;";
    $result = $connect->query($myQuery);
    }
  }
  else
...
<nav class="navbar">
  ...
  <!-- LOGO -->
  <div class="logo">View all customers</div>
...
<table id="reservations">
  ...
  <tr>
    ...
    <td><?php echo $rows['nationailty']; ?> </td>
